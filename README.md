...
从 [Releases](https://github.com/tkw6688/osu-AssetLinker/releases) 下载 `osu-AssetLinker.exe`。
...
输出文件位于 `publish/osu-AssetLinker.exe`。

## 用法

```
osu-AssetLinker [lazer目录] [Songs目录] [选项]
```
...
# 自动检测，并将已有普通文件替换为硬链接
osu-AssetLinker --relink

# 手动指定路径
osu-AssetLinker "C:\Users\你的用户名\AppData\Roaming\osu" "C:\osu!\Songs"

# 手动指定路径 + 重链接
osu-AssetLinker "D:\osu-lazer" "D:\osu-stable\Songs" --relink
```
